...
inline void fatalErrorException() {
...
Object::Object(ObjectMetadata &schema)
    : anchestor_(*this), metadata_(schema), isAlive_(isAncestor_) {
...
Object::Object(const Object &other)
    : anchestor_(*this), metadata_(other.metadata_), isAlive_(isAncestor_) {
    isAncestor_ = true;
}
...
uint8_t Object::getU8(std::string_view name) const {
    ...
    if (validate(name, DataType::UINT8, XXH3_64bits_const(name), offset,
                 unusedU64, unusedU64)) {
        memcpy(&value, buff_ + offset, sizeof(value));
    }
...
float Object::getF32(std::string_view name) const {
    float value;
    uint64_t offset;
    if (validate(name, DataType::FLOAT, XXH3_64bits_const(name), offset,
                 unusedU64, unusedU64)) {
        memcpy(&value, buff_ + offset, sizeof(value));
    }
...
double Object::getF64(std::string_view name) const {
    double value;
    uint64_t offset;
    if (validate(name, DataType::DOUBLE, XXH3_64bits_const(name), offset,
                 unusedU64, unusedU64)) {
        memcpy(&value, buff_ + offset, sizeof(value));
    }
...
std::string Object::getString(std::string_view name) const {
    ...
    if (validate(name, DataType::STRING, XXH3_64bits_const(name), offset, size,
                 unusedU64)) {
        value.resize(size);
        memcpy(value.data(), buff_ + offset, size);
...
void Object::setU8(std::string_view name, const uint8_t &value) const {
    ...
    if (validate(name, DataType::UINT8, XXH3_64bits_const(name), offset,
                 unusedU64, unusedU64)) {
        memcpy(buff_ + offset, &value, sizeof(value));
    }
}
...
void Object::setString(std::string_view name, const std::string &value) const {
    ...
    if (validate(name, DataType::STRING, XXH3_64bits_const(name), offset, size,
                 unusedU64)) {
        memcpy(buff_ + offset, value.c_str(),
               value.size() > size ? size : value.size());
        if (size > value.size()) {
            memset(buff_ + offset + value.size(), 0, size - value.size());
        }
    }
}
...
void Object::print(bool viewAllAncestor) {
    ...
    if (viewAllAncestor) {
        ...
        s << "package: " << metadata_.name << ", ID: unused"
...
void Object::print(std::stringstream &s, const std::string &prefix) const {
    for (int i = 0; i < metadata_.typeMetaData.size(); ++i) {
        ...
        s << std::left << std::setw(10) << (itr.name) << std::setw(14)
        ...
        switch (itr.type) {
            case DataType::UINT8:
                memcpy(&temp, buff_ + offset_ + itr.offset, sizeof(uint8_t));
                s << "value: " << temp.u64;
                break;
            case DataType::UINT16:
                memcpy(&temp, buff_ + offset_ + itr.offset, sizeof(uint16_t));
                s << "value: " << temp.u64;
                break;
            case DataType::UINT32:
                memcpy(&temp, buff_ + offset_ + itr.offset, sizeof(uint32_t));
                s << "value: " << temp.u64;
                break;
            case DataType::UINT64:
                memcpy(&temp, buff_ + offset_ + itr.offset, sizeof(uint64_t));
                s << "value: " << temp.u64;
                break;
            case DataType::INT8:
                memcpy(&temp, buff_ + offset_ + itr.offset, sizeof(int8_t));
                s << "value: " << temp.i64;
                break;
            case DataType::INT16:
                memcpy(&temp, buff_ + offset_ + itr.offset, sizeof(int16_t));
                s << "value: " << temp.i64;
                break;
            case DataType::INT32:
                memcpy(&temp, buff_ + offset_ + itr.offset, sizeof(int32_t));
                s << "value: " << temp.i64;
                break;
            case DataType::INT64:
                memcpy(&temp, buff_ + offset_ + itr.offset, sizeof(int64_t));
                s << "value: " << temp.i64;
                break;
            case DataType::FLOAT:
                memcpy(&temp, buff_ + offset_ + itr.offset, sizeof(float));
                s << "value: " << temp.f32;
                break;
            case DataType::DOUBLE:
                memcpy(&temp, buff_ + offset_ + itr.offset, sizeof(double));
                s << "value: " << temp.f64;
                break;
            case DataType::NA:
                break;
            case DataType::STRING:
                str.resize(itr.size);
                memcpy(str.data(), buff_ + offset_ + itr.offset, itr.size);
                s << "value: " << str;
                break;
